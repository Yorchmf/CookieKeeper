...
| 16a | ☐ **`region`-scoped Consent Mode defaults** (deferred from #16) | Consent Mode can vary defaults by region with no geo-IP and no PII (Google geolocates), so it looked like free EEA-only strictness. It is not free: the *value* is granting by default outside the EEA, and doing that while still rendering a consent banner to that same visitor is self-contradictory — we would be telling them they have a choice and telling Google they already consented. Making it coherent means the banner itself must be geo-targeted, which is a real feature (which regions see a banner, what the fallback is when geolocation is unavailable, how the consent log records "not shown"), not a flag on the default push. Revisit as part of banner geo-targeting, not as a widget one-liner. |
