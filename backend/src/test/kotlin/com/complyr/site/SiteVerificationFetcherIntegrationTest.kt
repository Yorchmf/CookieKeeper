...
import com.complyr.scan.ScanFailureReason
...
class SiteVerificationFetcherIntegrationTest {
    ...
    private fun fetcher(
        verification: ComplyrProperties.Verification = ComplyrProperties.Verification(),
     ...
     */
    private fun fetch(verification: ComplyrProperties.Verification = ComplyrProperties.Verification()): String? =
    ...
    fun `truncates an oversized body at the cap instead of buffering it all`() {
        ...
        val body = fetch(ComplyrProperties.Verification(maxBodyBytes = cap))
    ...
    fun `gives up on a redirect loop after the configured number of hops`() {
        ...
        assertNull(fetch(ComplyrProperties.Verification(maxRedirects = 3)))
    ...
    private fun stallingConfig() =
        ComplyrProperties.Verification(
    ...
    fun `treats a non-html content type as a miss`() {
        handle("/") {
            ...
            val body = """{"data":"<script src=\"https://cdn.complyr.eu/v1.js\"></script>"}""".toByteArray()
