...
class AuthEmailComposerTest {
    ...
    fun `welcome email links to the localized dashboard home`() {
        ...
        assertTrue(
            email.htmlBody.contains("https://app.complyr.eu/de"),
    ...
    fun `welcome email resolves a non-blank subject and correct link for every supported locale`() {
        supportedLocales.forEach { locale ->
            ...
            assertTrue(
                email.htmlBody.contains("https://app.complyr.eu/$locale"),
    ...
    fun `verification and reset emails still embed the tokenized link`() {
        ...
        assertTrue(
            verification.htmlBody.contains("https://app.complyr.eu/en/verify-email?token=raw-verify-token"),
        )
        assertTrue(
            reset.htmlBody.contains("https://app.complyr.eu/en/reset-password?token=raw-reset-token"),
        )
    }
}
