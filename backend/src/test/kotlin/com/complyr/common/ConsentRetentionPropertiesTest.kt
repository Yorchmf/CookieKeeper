...
class ConsentRetentionPropertiesTest {
    ...
    fun `defaults to 36 months (the longest plan retention)`() {
        assertEquals(36, ComplyrProperties.Consent().retentionMonths)
    }

    @Test
    fun `accepts the floor and larger windows`() {
        assertEquals(
            ComplyrProperties.Consent.MIN_RETENTION_MONTHS,
            ComplyrProperties.Consent(retentionMonths = ComplyrProperties.Consent.MIN_RETENTION_MONTHS).retentionMonths,
        )
        // A longer window (e.g. toward the 5-yr CLAUDE.md ceiling) is always allowed.
        assertEquals(60, ComplyrProperties.Consent(retentionMonths = 60).retentionMonths)
    }
    ...
    fun `rejects a window below the longest plan retention`() {
        ...
        assertThrows<IllegalArgumentException> { ComplyrProperties.Consent(retentionMonths = 35) }
        assertThrows<IllegalArgumentException> { ComplyrProperties.Consent(retentionMonths = 12) }
        assertThrows<IllegalArgumentException> { ComplyrProperties.Consent(retentionMonths = 0) }
    }
    ...
    fun `floor never drifts below any plan's entitled retention`() {
        ...
        assertTrue(
            ComplyrProperties.Consent.MIN_RETENTION_MONTHS >= longestPlanRetentionMonths,
            "MIN_RETENTION_MONTHS (${ComplyrProperties.Consent.MIN_RETENTION_MONTHS}) must be >= the " +
