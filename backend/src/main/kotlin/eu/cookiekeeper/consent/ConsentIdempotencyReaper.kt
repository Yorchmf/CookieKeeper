 ...
 * anything older than [CookieKeeperProperties.Consent.idempotencyRetention] is safe to delete — a key
 ...
 * deletes the oldest [CookieKeeperProperties.Consent.idempotencyPruneBatchSize] rows per transaction
...
class ConsentIdempotencyReaper(
     ...
     * [CookieKeeperProperties.Consent.idempotencyPruneBatchSize], until the window is drained or the
     * per-run batch cap is hit. Cron-scheduled off-peak; overridable via
     * `cookiekeeper.consent.idempotency-prune-cron` (defaulted here so no yml entry is required).
     *
