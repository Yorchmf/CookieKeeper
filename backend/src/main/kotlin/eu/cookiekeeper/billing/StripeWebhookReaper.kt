 ...
 * evidence, so DELETE is the right tool. Structure mirrors [eu.cookiekeeper.scan.PublicScanReaper]: one
 ...
 * is hit. (See [eu.cookiekeeper.common.SchedulingConfig] for why any shared-state `@Scheduled` job must
 * leader-guard itself.)
 */
@Component
class StripeWebhookReaper(
     ...
     * cap is hit. Cron-scheduled off-peak; overridable via `cookiekeeper.billing.stripe-event-prune-cron`
