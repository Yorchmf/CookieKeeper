 ...
 * preset (a 90-day view compares against days 90–180 back). [CookieKeeperProperties.Impression.retention]
 ...
 * or preserve. Mirrors [eu.cookiekeeper.consent.ConsentIdempotencyReaper] in every operational
 ...
 * deletes the oldest [CookieKeeperProperties.Impression.pruneBatchSize] rows per transaction and loops
...
class BannerImpressionReaper(
     ...
     * [CookieKeeperProperties.Impression.pruneBatchSize], until the window is drained or the per-run
     * batch cap is hit. Cron-scheduled off-peak; overridable via
     * `cookiekeeper.impression.prune-cron` (defaulted here so no yml entry is required).
     *
     * The cutoff is a whole UTC calendar day matching the counter's own `day` grain and the day
     * the ingestion service stamps ([eu.cookiekeeper.consent.ImpressionService]).
     *
     ...
     */
    @Scheduled(cron = "\${cookiekeeper.impression.prune-cron:$DEFAULT_PRUNE_CRON}")
    fun prune() {
