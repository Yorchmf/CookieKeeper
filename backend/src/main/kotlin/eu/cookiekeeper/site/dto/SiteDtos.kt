...
import eu.cookiekeeper.site.DnsTxtLookup
...
/**
 * Set the site's "hide the Powered by CookieKeeper credit" preference. Nullable + `@NotNull` so a missing
 ...
 * this endpoint never grants the paid feature on its own (see [eu.cookiekeeper.billing.EntitlementService]).
 */
 ...
 * what [eu.cookiekeeper.site.DnsTxtLookup] actually queries.
 */
data class SiteDetailResponse(
    ...
    /** The customer's saved preference to hide the "Powered by CookieKeeper" credit. */
 ...
 * an error — see [eu.cookiekeeper.site.SiteVerificationService] for why that distinction is load-bearing
