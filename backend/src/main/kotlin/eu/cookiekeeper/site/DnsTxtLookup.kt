...
/**
 * The DNS-TXT half of domain verification (ADR-17): a `_cookiekeeper.{domain}` TXT record whose value is the
 ...
 *     is the constant `_cookiekeeper.` prefix: any scheme an attacker tried to smuggle in would have to be
 *     `_cookiekeeper.<x>`, which resolves to an unloadable factory class name and falls back to the DNS
...
class DnsTxtLookup(
    ...
    /**
     * True when `_cookiekeeper.{domain}` publishes a TXT record equal to [siteKey]. Any failure — NXDOMAIN,
    ...
    fun hasSiteKeyRecord(
        ...
        // A blank key would match an empty TXT record (`_cookiekeeper.victim.com. IN TXT ""` decodes to `""`),
    ...
    /**
     * `_cookiekeeper.{domain}`, or null if [domain] is not a domain. Re-normalizing here rather than trusting
    ...
    /**
     * The DNS seam. Mirrors [eu.cookiekeeper.scan.ScanTargetValidator]'s `HostResolver`: production binds the
