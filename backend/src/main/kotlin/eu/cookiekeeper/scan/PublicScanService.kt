...
import eu.cookiekeeper.site.DomainValidator
 ...
 *
 * Abuse posture (slice D): three layers guard crawl compute. The per-IP [eu.cookiekeeper.common.RateLimitFilter]
 * `PUBLIC_SCAN` tier brakes request *rate* upstream of this service; a **honeypot** field short-circuits
 * naive bots to a no-op here; and a per-requester **concurrency cap** ([CookieKeeperProperties.Scan.maxConcurrentScansPerIp])
...
class PublicScanService(
     ...
     * Throws [eu.cookiekeeper.site.InvalidDomainException] (400) for a non-public domain and
    ...
    /**
     * Reject the enqueue when this requester already has [CookieKeeperProperties.Scan.maxConcurrentScansPerIp]
