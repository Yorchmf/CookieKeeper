...
/**
 * Makes each plan's [eu.cookiekeeper.billing.RescanFrequency] real: nightly it enqueues a fresh scan for
 ...
 * unverified site simply crawls in [eu.cookiekeeper.scan.CrawlMode.QUICK] (identical posture to the anonymous
 ...
 * [eu.cookiekeeper.billing.EXPIRED_ENTITLEMENTS]. Note this does NOT touch consent ingestion — CLAUDE.md
 ...
 * [CookieKeeperProperties.Scan.rescanBatchSize] cap so a backlog drains over nights, and per-site jitter that
 * spreads each job's `available_at` across [CookieKeeperProperties.Scan.rescanJitterWindow]. The single Chromium
 * worker and [CookieKeeperProperties.Scan.maxJobsPerPoll] are the final backstop.
 */
@Component
class ScheduledRescanJob(
     ...
     * overridable via `cookiekeeper.scan.rescan-cron` (defaulted here so no yml entry is required).
     *
