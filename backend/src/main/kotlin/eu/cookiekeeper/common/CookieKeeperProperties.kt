...
/**
 * Typed, immutable view of the `cookiekeeper.*` configuration tree (application.yml / env vars).
 */
@ConfigurationProperties("cookiekeeper")
data class CookieKeeperProperties(
    val auth: Auth,
    ...
    val mailFrom: String,
    // Destination for in-app support contact-form messages (eu.cookiekeeper.support). Unlike [mailFrom] this
    // carries a safe default so the 21 unit tests that build CookieKeeperProperties by hand need not each name
    ...
    companion object {
        const val DEFAULT_SUPPORT_INBOX = "support@cookiekeeper.eu"
    }
    ...
    data class Auth(
        ...
        // (see [eu.cookiekeeper.auth.LoginAttemptService]). The lock is TEMPORARY (auto-expiring) rather than
        ...
        init {
            require(jwtSecret.toByteArray(Charsets.UTF_8).size >= MIN_JWT_SECRET_BYTES) {
                "cookiekeeper.auth.jwt-secret must be at least $MIN_JWT_SECRET_BYTES bytes (HS256 key material)"
            }
            ...
            require(maxFailedLoginAttempts > 0) {
                "cookiekeeper.auth.max-failed-login-attempts must be positive (was $maxFailedLoginAttempts)"
            }
            ...
            require(!loginLockoutDuration.isZero && !loginLockoutDuration.isNegative) {
                "cookiekeeper.auth.login-lockout-duration must be a positive duration (was $loginLockoutDuration)"
            }
            ...
            require(!emailChangeTokenTtl.isZero && !emailChangeTokenTtl.isNegative) {
                "cookiekeeper.auth.email-change-token-ttl must be a positive duration (was $emailChangeTokenTtl)"
            }
        }
    ...
    data class RateLimit(
        ...
        // concurrency cap (cookiekeeper.scan.max-concurrent-scans-per-ip) and edge limiting back it up.
        ...
        // JWT subject — see [eu.cookiekeeper.common.AuthenticatedRateLimitFilter]). Tight: every billing
    ...
    /**
     * CORS policy for the public widget endpoints (see [eu.cookiekeeper.common.CorsConfig]).
     * All fields are optional in `cookiekeeper.cors.*`; when unset the defaults below apply —
     ...
     * [eu.cookiekeeper.consent.ConsentIdempotencyReaper]). It only has to outlive a pending widget
     ...
     * its bloat (see V5 migration). The prune schedule itself is the raw `cookiekeeper.consent.
     ...
     * dead tuples — see [eu.cookiekeeper.consent.ConsentIdempotencyReaper].
     *
     * [originTokenSecret] / [originTokenTtl] configure the stateless HMAC consent-origin token (see
     * [eu.cookiekeeper.consent.ConsentOriginToken]). The secret is CONFIGURED, not random-per-process,
     ...
     * tests still constructs — the [eu.cookiekeeper.consent.ConsentOriginToken] bean rejects a secret
    ...
    data class Consent(
        ...
        companion object {
            ...
            // [eu.cookiekeeper.consent.ConsentEventPartitionProvisioner].
            ...
            // ([eu.cookiekeeper.consent.ConsentEventPartitionReaper]) drops any monthly partition older
            ...
            // and kept honest by a drift guard in CookieKeeperPropertiesTest.
            const val MIN_RETENTION_MONTHS = 36
        }
    }

    /**
     * Banner-impression counter tuning (Track 4 Slice D; see [eu.cookiekeeper.analytics.BannerImpressionReaper]).
     *
     ...
     * keep. The prune schedule itself is the raw `cookiekeeper.impression.prune-cron` property read by
    ...
    /**
     * Scan queue tuning (see [eu.cookiekeeper.scan.ScanQueue] / [eu.cookiekeeper.scan.ScanWorker]).
     *
     ...
     *
     * The poll interval itself is read directly by `@Scheduled` from `cookiekeeper.scan.poll-interval-millis`
    ...
    data class Scan(
        ...
        // available; see [eu.cookiekeeper.scan.PublicScanService].
        val maxConcurrentScansPerIp: Int = DEFAULT_MAX_CONCURRENT_SCANS_PER_IP,
        // Rows the retention reaper deletes per transaction (see [eu.cookiekeeper.scan.PublicScanReaper]).
        ...
        // `cookiekeeper.scan.public-scan-prune-cron` property read by `@Scheduled`, not typed here.
        val publicScanPruneBatchSize: Int = DEFAULT_PUBLIC_SCAN_PRUNE_BATCH_SIZE,
        // Scheduled re-scan job tuning (ADR-17; see [eu.cookiekeeper.scan.ScheduledRescanJob]).
        ...
        // `cookiekeeper.scan.rescan-cron` property read by `@Scheduled`, not typed here.
     ...
     * [eu.cookiekeeper.site.SiteVerificationFetcher] and the TXT lookup in
     ...
     * untouched (see [eu.cookiekeeper.billing.PlanResolver]). [trialConsentEventCap] bounds consent
     ...
     * (see [eu.cookiekeeper.billing.StripeConfig] / [eu.cookiekeeper.billing.BillingService]). They are bound
     ...
     * [eu.cookiekeeper.billing.StripeApiGateway.parseWebhookEvent] to verify every inbound webhook. Like
     ...
     * [eu.cookiekeeper.billing.StripeWebhookReaper]). Retention only has to outlive Stripe's redelivery
     ...
     * `cookiekeeper.billing.stripe-event-prune-cron` property read by `@Scheduled`, not typed here.
     *
    ...
    data class Billing(
        ...
        // with `cookiekeeper.app-base-url` into absolute URLs by [eu.cookiekeeper.billing.BillingService].
         ...
         * plan→price mapping lives in [eu.cookiekeeper.billing.BillingService] so this config-binding
    ...
    /**
     * Transactional email delivery selection. [provider] picks exactly one [eu.cookiekeeper.notify.EmailSender]
     ...
     * (SMTP path, and unrelated unit tests) still constructs; the [eu.cookiekeeper.notify.BrevoEmailSender]
    ...
    /**
     * Error-tracking (Sentry) configuration — see [eu.cookiekeeper.common.SentryConfig] and ADR-15.
     *
     ...
     * DSN (host `*.de.sentry.io`); the [eu.cookiekeeper.common.SentryConfig] bean refuses a non-EU DSN at
