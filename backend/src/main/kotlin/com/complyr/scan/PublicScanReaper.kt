 ...
 * Instead it deletes the oldest-expiring [ComplyrProperties.Scan.publicScanPruneBatchSize] rows per
 ...
 * stops; the winner drains the backlog. (See [com.complyr.common.SchedulingConfig] for why any new
...
class PublicScanReaper(
     ...
     * [ComplyrProperties.Scan.publicScanPruneBatchSize], until the window is drained or the per-run
     * batch cap is hit. Cron-scheduled off-peak; overridable via `complyr.scan.public-scan-prune-cron`
    ...
    companion object {
         ...
         * prune across instances. Kept distinct from [com.complyr.consent.ConsentIdempotencyReaper]'s
